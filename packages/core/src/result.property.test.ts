...
describe('Diff Result Builder Property Tests', () => {
  ...
  it('Property 6: 统计信息一致性', () => {
    fc.assert(
      fc.property(fc.jsonValue(), fc.jsonValue(), (oldValue, newValue) => {
        ...
        // 验证总节点数等于各类型之和
        const totalNodes = result.stats.added + result.stats.deleted + 
                          result.stats.modified + result.stats.unchanged;
  ...
  it('Property 6 (补充): computeStats 对手动构建的节点正确工作', () => {
    fc.assert(
      fc.property(
        fc.array(fc.constantFrom(DiffType.ADDED, DiffType.DELETED, DiffType.MODIFIED, DiffType.UNCHANGED)),
        (types) => {
          // 手动构建一个包含指定类型节点的树
          ...
          const root = DiffResultBuilder.createModifiedNode(
            [],
            'object' as any,
            {},
            {},
  ...
  it('Property 6 (补充): 嵌套结构的统计信息正确', () => {
    fc.assert(
      fc.property(fc.object({ maxDepth: 3 }), fc.object({ maxDepth: 3 }), (oldValue, newValue) => {
        ...
        // 验证统计信息的总和等于节点总数
        const totalFromStats = result.stats.added + result.stats.deleted + 
                              result.stats.modified + result.stats.unchanged;
